...
// ブロック伝搬重複抑止
// ============================================================
function isBlockSeen(hash) {
    return seenBlockHashes.has(hash);
}
function markBlockSeen(hash) {
...
setInterval(() => {
    ...
    for (const [hash, time] of seenBlockHashes) {
        if (now - time > CONFIG.BLOCK_HASH_TTL)
            seenBlockHashes.delete(hash);
    }
}, 30000);
// ============================================================
// seeds.json
// ============================================================
async function fetchSeedsFromCDN() {
...
function loadSeedsLocal() {
    try {
        if (fs.existsSync(CONFIG.SEEDS_PATH)) {
            const file = JSON.parse(fs.readFileSync(CONFIG.SEEDS_PATH, 'utf-8'));
            return file.seeds;
        }
    }
    catch { }
...
// ============================================================
async function connectToRandomSeed() {
    // CDNから最新取得
    try {
...
function reconnectToRandomSeed(baseDelay) {
...
// ============================================================
function handleExplorerAPI(req, res) {
    ...
    if (url?.startsWith('/api/block/')) {
        ...
        // 一時的にHTTPレスポンスを保持
        globalThis.__pendingAPI = globalThis.__pendingAPI || new Map();
        globalThis.__pendingAPI.set(reqId, { res, timeout: setTimeout(() => {
                globalThis.__pendingAPI.delete(reqId);
...
function startWSSServer() {
    ...
    wss.on('connection', (ws) => {
        ...
        const conn = { ws, id: clientId, connectedAt: Date.now() };
