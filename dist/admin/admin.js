...
// node.js配信機能
// ============================================================
window.deployNodeCode = async function () {
    if (!isAuthenticated || adminWallet?.role !== 'root') {
        addLog('systemLog', 'node.js配信にはroot権限が必要です', 'error');
        return;
    }
    // ★ ROOT_KEY確認
    const ROOT_KEY = '04920517f44339fed12ebbc8f2c0ae93a0c2bfa4a9ef4bfee1c6f12b452eab70';
    ...
    const fileInput = document.getElementById('nodeCodeFile');
    const versionInput = $val('nodeCodeVersion');
