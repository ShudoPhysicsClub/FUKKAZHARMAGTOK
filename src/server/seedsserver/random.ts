...
interface RandomCommit {
  nodeId: string;
  hash: string;     // hash(乱数)
}

interface RandomReveal {
  nodeId: string;
  value: string;    // 乱数の実値
}

export class RandomManager {
  private currentRandom: string = '';
  private commits: Map<string, RandomCommit> = new Map();
  private reveals: Map<string, RandomReveal> = new Map();
  private selectedNodes: string[] = [];
  private phase: 'idle' | 'commit' | 'reveal' = 'idle';
   ...
   */
  startRound(activeNodes: NodeInfo[]): { selectedNodes: string[]; fallback: boolean } {
    this.commits.clear();
    this.reveals.clear();
    this.selectedNodes = [];
  ...
  receiveCommit(nodeId: string, hash: string): boolean {
    ...
    if (!this.selectedNodes.includes(nodeId)) return false;
   ...
   * フォールバック乱数生成
   */
  private generateFallbackRandom(): string {
