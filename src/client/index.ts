...
let miningTargetToken: string = '';  // 空=BTRのまま、アドレス指定=掘ったらswap
  ...
  switch (packet.type) {
    ...
    case 'chain_chunk': {
      ...
      if (blocks && blocks.length > 0) {
        ...
        const newHeight: number = lastBlock.height + 1;
    ...
    case 'new_block': {
      ...
      if (!blk) break;
      const newHeight: number = (blk.height || 0) + 1;
      if (newHeight > chainHeight) chainHeight = newHeight;
      ...
      if (wallet && blk.miner === wallet.address) break;
      // 他人のブロック
...
async function createWallet(): Promise<void> {
  ...
  try {
    const pubBytes: Uint8Array = await Ed25519.getPublicKey(privateKeyBytes);
    const publicKey: string = bytesToHex(pubBytes);
    const address: string = '0x' + (await sha256(publicKey)).slice(0, 40);
...
async function importWallet(): Promise<void> {
  const key: string = $val('importKey');
  if (key.length !== 64) {
  ...
  try {
    const privBytes: Uint8Array = hexToBytes(key);
    const pubBytes: Uint8Array = await Ed25519.getPublicKey(privBytes);
    const publicKey: string = bytesToHex(pubBytes);
    const address: string = '0x' + (await sha256(publicKey)).slice(0, 40);
...
function exportWallet(): void {
  ...
  const data: string = `秘密鍵: ${wallet.privateKey}\n公開鍵: ${wallet.publicKey}\nアドレス: ${wallet.address}`;
...
async function loadWallet(): Promise<void> {
  const saved: string | null = localStorage.getItem('btr_wallet');
  if (!saved) return;
  try {
...
function updateBalanceUI(): void {
  ...
  const tokenKeys: string[] = Object.keys(tokenBalances).filter(addr => tokenBalances[addr] > 0);
...
async function signAndSend(txData: Record<string, unknown>): Promise<void> {
  ...
  const msgBytes: Uint8Array = new TextEncoder().encode(message);

  try {
    const privBytes: Uint8Array = hexToBytes(wallet.privateKey);
    const sigBytes: Uint8Array = await Ed25519.sign(msgBytes, privBytes);
...
async function sendBTR(): Promise<void> {
  const to: string = $val('sendTo');
  const amount: number = parseFloat($val('sendAmount'));
  if (!to || isNaN(amount) || amount <= 0) { addLog('globalLog', '宛先と金額を入力', 'error'); return; }
  await signAndSend({ type: 'transfer', token: BTR_ADDRESS, to, amount });
}

async function sendToken(): Promise<void> {
  const token: string = $val('tokenSendToken');
  const to: string = $val('tokenSendTo');
  const amount: number = parseFloat($val('tokenSendAmount'));
  if (!token || !to || isNaN(amount) || amount <= 0) { addLog('globalLog', '全項目を入力', 'error'); return; }
  await signAndSend({ type: 'token_transfer', token, to, amount });
}

async function createToken(): Promise<void> {
  const symbol: string = $val('tokenSymbol');
  const name: string = $val('tokenName');
  const totalSupply: number = parseInt($val('tokenSupply'));
  const poolRatio: number = parseFloat($val('tokenPoolRatio')) || 0;
  const distribution: string = $val('tokenDist');
  if (!symbol || !name || isNaN(totalSupply) || totalSupply <= 0) { addLog('globalLog', '全項目を入力', 'error'); return; }
...
async function executeSwap(): Promise<void> {
  const tokenIn: string = $val('swapIn') || BTR_ADDRESS;
  const tokenOut: string = $val('swapOut');
  const amountIn: number = parseFloat($val('swapAmount'));
  if (!tokenOut || isNaN(amountIn) || amountIn <= 0) { addLog('globalLog', '全項目を入力', 'error'); return; }
...
function searchToken(): void {
  const query: string = $val('tokenSearch').toUpperCase();
  if (!query) { $('tokenSearchResults').innerHTML = '<div style="color:var(--text2);font-size:12px">シンボルを入力してください</div>'; return; }
...
function showTokenSearchResults(query: string, allTokens: { address: string; symbol: string; name: string; totalSupply: number }[]): void {
  ...
  $('tokenSearchResults').innerHTML = matches.map(t =>
    `<div class="token-item" style="flex-direction:column;gap:4px;cursor:pointer" data-addr="${t.address}">
...
function startMining(): void {
  if (!wallet) { addLog('miningLog', 'ウォレットを先に作成してください', 'error'); return; }
  if (!ws || ws.readyState !== WebSocket.OPEN) { addLog('miningLog', '接続されていません', 'error'); return; }
  isMining = true;
  miningStartTime = Date.now();
  totalHashes = 0;
  ...
  ($('miningStatus') as HTMLElement).style.color = 'var(--accent)';
...
function stopMining(): void {
  ...
  ($('miningStatus') as HTMLElement).style.color = '';
...
function bindEvents(): void {
  ...
  $('nav').addEventListener('click', (e: Event) => {
    ...
    const panel: string | undefined = target.dataset.panel;
    if (!panel) return;
  ...
  $('tokenSearch').addEventListener('keydown', (e: Event) => {
    if ((e as KeyboardEvent).key === 'Enter') searchToken();
  });
  // スワップ入力変更時にレート取得
