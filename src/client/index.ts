...
const SEEDS_CDN: string = 'https://cdn.jsdelivr.net/gh/ShudoPhysicsClub/FUKKAZHARMAGTOK@main/src/server/fullserver/seeds.json';
const WEI_PER_BTR = 1_000_000_000_000_000_000n;
const GAS_FEE_WEI: string = (1n * WEI_PER_BTR).toString();

interface SeedEntry { host: string; id: number; }
let seedsList: SeedEntry[] = [];
let currentSeedHost: string = '';
let wsReconnectDelay: number = 1000;
...
function showToast(message: string, type: 'success' | 'error' | 'info' = 'info', duration: number = 4000): void {
...
function getRandomSeedURL(): string {
  if (seedsList.length === 0) return 'wss://mail.shudo-physics.com:443';
  // 現在接続中以外からランダム選択
...
function loadQRious(): Promise<void> {
  return new Promise((resolve, reject) => {
    if (typeof (window as any).QRious !== 'undefined') { resolve(); return; }
...
async function showQR(): Promise<void> {
  ...
  try {
    ...
    new (window as any).QRious({ element: canvas, value: qrData, size: 220, background: '#ffffff', foreground: '#000000' });
...
function parsePaymentURL(urlStr: string): { user: string; value: string } | null {
  try {
    // 完全なURLの場合
    const url = new URL(urlStr);
    const user = url.searchParams.get('user');
    if (user) return { user, value: url.searchParams.get('value') || '' };
  } catch {
...
function applyPaymentParams(user: string, value: string): void {
  switchTab('send');
  const sendToInput = $('sendTo') as HTMLInputElement;
  if (sendToInput) sendToInput.value = user;
  if (value && parseFloat(value) > 0) {
    const sendAmountInput = $('sendAmount') as HTMLInputElement;
    if (sendAmountInput) sendAmountInput.value = value;
  }
  ...
  <div class="panel" id="panel-mining">
    <div class="card"><h2>マイニング</h2>
      <div class="mining-stats">
        <div class="stat-box"><div class="label">ステータス</div><div class="value" id="miningStatus">あいどんとのー</div></div>
